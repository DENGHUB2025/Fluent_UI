...
local SaveManager = {} do
	...
	function SaveManager:Save(name)
		...
		local fullPath = self:GetUserFolder() .. "/settings/" .. name .. ".json"
		local data = { objects = {} }
	...
	function SaveManager:Load(name)
		...
		local file = self:GetUserFolder() .. "/settings/" .. name .. ".json"
	...
	function SaveManager:LoadAutoloadConfig()
		local autoLoadFile = self:GetUserFolder() .. "/settings/autoload.txt"
	...
	function SaveManager:EnableAutoSave(configName)
		self.AutoSaveName = configName or "autosave"
		
		for idx, option in pairs(self.Options) do
			local originalSetValue = option.SetValue
	
			option.SetValue = function(obj, ...)
				originalSetValue(obj, ...)
				pcall(function()
	...
	function SaveManager:BuildConfigSection(tab)
		...
		AutoloadButton = section:AddButton({Title = "Set as autoload", Description = "Current autoload config: none", Callback = function()
			local name = SaveManager.Options.SaveManager_ConfigList.Value
			writefile(self:GetUserFolder() .. "/settings/autoload.txt", name)
		...
		local userAutoLoad = self:GetUserFolder() .. "/settings/autoload.txt"
		if isfile(userAutoLoad) then
			local name = readfile(userAutoLoad)
			AutoloadButton:SetDesc("Current autoload config: " .. name)
		end
